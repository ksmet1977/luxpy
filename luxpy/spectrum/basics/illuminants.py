...
#------------------------------------------------------------------------------
def _get_daylightlocus_parameters(ccts, spds, cieobs, cct_is_nominal = False, interp_settings = None):
    """
    ...
    if cct_is_nominal: # account for change in c2 in def. of Planckian
        cct_scaler = (1.4388/1.4380)
        ccts = ccts*cct_scaler
    else:
        cct_scaler = 1.0
...
def daylightlocus(cct, force_daylight_below4000K = False, cieobs = None, daylight_locus = None,
                  cct_is_nominal = False, use_published_daylightlocus_coeffs_when_cieobs_is_1931_2 = True, 
    ...
    if (cieobs is None) | (use_published_daylightlocus_coeffs_when_cieobs_is_1931_2 & (cieobs == '1931_2')): # use default values for '1931_2' reported in CIE15-20xx
        ...
        p = cct>=(7000.0*cct_scaler)
        xD[p] = -2.0064*((1.0e3/cct[p])**3.0)+1.9018*((1.0e3/cct[p])**2.0)+0.24748*(1.0e3/cct[p])+0.23704
        yD = -3.0*xD**2.0+2.87*xD-0.275

    else:
        if isinstance(cieobs, str):
            if (daylight_locus is None) & (cct_is_nominal == False):
                daylight_locus = _DAYLIGHT_LOCI_PARAMETERS[cieobs]
            elif (daylight_locus is None) & (cct_is_nominal == True): # calculate on-the-spot for nominal ccts.
                daylight_locus = get_daylightloci_parameters(cieobs = [cieobs], cct_is_nominal = cct_is_nominal,
...
def _get_daylightphase_Mi_values(xD,yD, Mcoeffs = None, cieobs = None, S012_daylightphase = None,
    ...
    Args:
        ...
        :cct_is_nominal:
            | False, optional
            | If True: Round M1, M2 values to 3 decimals as recommended by CIE 
            | when calculating daylight phases for nominal CCTs (e.g. 5500 K, 6500 K) 
    ...
    if cct_is_nominal:
        rr = 3 # rounding of Mi for nominal CCTs
        M1 = np.round(M1, rr)
        M2 = np.round(M2, rr)
...
#------------------------------------------------------------------------------
def daylightphase(cct, wl3 = None, cct_is_nominal = False, force_daylight_below4000K = False, verbosity = None, 
    ...
    if (cct < (4000.0)) & (force_daylight_below4000K == False):
        ...
        # Calculate weighted combination of S0, S1 & S2 components:
...
def get_daylightloci_parameters(ccts = None, cieobs = None, wl3 = [300,830,10], verbosity = 0, cct_is_nominal = False,
    ...
    for cieobs_ in cieobs:
        ...
        xy, pxy, pxT_l7, pxT_L7, l7, L7 = _get_daylightlocus_parameters(ccts, spds, cieobs_, cct_is_nominal = cct_is_nominal, interp_settings = interp_settings)
