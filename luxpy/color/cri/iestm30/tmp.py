...
def plot_tm30_spd(spd, cri_type = 'ies-tm30', axh = None, **kwargs):
    ...
    axh.set_xlabel('Wavelength (nm)', fontsize = _FONTSIZE)
    axh.set_ylabel('Radiant power\n(Equal Luminous Flux)', fontsize = _FONTSIZE)
    axh.set_xlim([360,830]) 
    axh.set_yticklabels([])
    axh.legend(loc = 'upper right', fontsize = _FONTSIZE)
...
def plot_tm30_Rfi(spd, cri_type = 'ies-tm30', axh = None, **kwargs):
    ...
    axh.set_xticklabels(xtickslabels, fontsize = _FONTSIZE, rotation = 90)
    axh.set_ylabel(r'Color Sample Fidelity $(R_{f,CESi})$', fontsize = _FONTSIZE)
    axh.set_ylim([0,100])
...
def plot_tm30_Rfhi(spd, cri_type = 'ies-tm30', axh = None, xlabel = True, y_offset = 0, **kwargs):
    """
    Plot Local Color Fidelity values (Rfhi) (one for each hue-bin).
    ...
    for j in hbins:
        ...
        axh.text(hbins[j],ypos, '{:1.0f}'.format(Rfhi[j,0]) ,fontsize = _FONTSIZE,horizontalalignment='center',verticalalignment='center',color = np.array([1,1,1])*0.3)
    
    xticks = np.array(hbins)
    axh.set_xticks(xticks)
    if xlabel == True:
        xtickslabels = ['{:1.0f}'.format(ii+1) for ii in hbins]
        axh.set_xlabel('Hue-Angle Bin (j)', fontsize = _FONTSIZE)
    else:
        xtickslabels = [''.format(ii+1) for ii in hbins]
    axh.set_xticklabels(xtickslabels, fontsize = _FONTSIZE)
    axh.set_xlim([-0.5,data['nhbins']-0.5])
    
    axh.set_ylabel(r'Local Color Fidelity $(R_{f,hj})$', fontsize = _FONTSIZE)
    axh.set_ylim([0,110])
...
def plot_tm30_Rcshi(spd, cri_type = 'ies-tm30', axh = None, xlabel = True, y_offset = 0, **kwargs):
    """
    Plot Local Chroma Shift values (Rcshi) (one for each hue-bin).
    ...
    for j in hbins:
        ...
        axh.text(hbins[j],ypos, '{:1.0f}%'.format(100*Rcshi[j,0]), fontsize = _FONTSIZE,horizontalalignment='center',verticalalignment='center',color = np.array([1,1,1])*0.3, rotation = 90)
    ...
    axh.set_yticklabels(ytickslabels, fontsize = _FONTSIZE)
    axh.set_ylabel(r'Local Chroma Shift $(R_{cs,hj})$', fontsize = _FONTSIZE)
...
def plot_tm30_Rhshi(spd, cri_type = 'ies-tm30', axh = None, xlabel = True, y_offset = 0, **kwargs):
    """
    Plot Local Hue Shift values (Rhshi) (one for each hue-bin).
    ...
    for j in hbins:
        ...
        axh.text(hbins[j],ypos, '{:1.2f}'.format(Rhshi[j,0]) ,fontsize = _FONTSIZE,horizontalalignment='center',verticalalignment='center',color = np.array([1,1,1])*0.3, rotation = 90)
...
def plot_tm30_Rxhi(spd, cri_type = 'ies-tm30', axh = None, **kwargs):
    """
    Plot Local Chroma Shifts (Rcshi), Local Hue Shifts (Rhshi) and Local Color Fidelity values (Rfhi) (one for each hue-bin).
...
def plot_tm30_reportb(spd, cri_type = 'ies-tm30',
    ...
    _plot_tm30_report_top(f_ax_top, source = source, manufacturer = manufacturer,
                   date = date, model = model)
    _plot_tm30_report_bottom(f_ax_bottom, spd, notes = notes)
