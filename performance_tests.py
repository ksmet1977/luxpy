...
def analyze_performance(performance, perf_strs, MN, write_to_xls=False):
    # Python: store in dict:
    n = list(perf_strs.keys())
    keys = list(perf_strs.values())
    #performance_=performance[:,:,n]
    perf_dict_py=lx.odict()
    for i in range(len(n)):
        if not ("..." in keys[i]):
            perf_dict_py[keys[i]] = performance[:,:,n[i]]
    ...
    if write_to_xls == True:
        ...
        for i in range(len(perf_dict_py.keys())):
            lx.write_to_excel("performance_comp_py1e4_jl1e4.xlsx",lx.pd.DataFrame(getkv(i)[1],columns = spdsize,index=spdsize),getkv(i)[0])
      
    return perf_dict_pydivjl, getkv


if __name__ == '__main__':
    do_performance_tests = False
