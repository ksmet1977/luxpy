...
__all__ = ['_cspace_axes', '_ipt_M','todimT','asplitT','ajoinT','xyz_to_YxyT','Yxy_to_xyzT','xyz_to_YuvT','Yuv_to_xyzT',
           'xyz_to_wuv','wuv_to_xyz','xyz_to_xyz','xyz_to_labT','lab_to_xyzT','xyz_to_luv','luv_to_xyz',
...
#---------------------------------------------------------------------------------------------------
def asplitT(data, axis = None):
    """
    Split np.array data on (last) axis
    """
    if axis is None:
        axis = 0
    return np.array([data.take([x],axis = axis) for x in range(data.shape[axis])])


def ajoinT(data, axis = None):
    """
...
def todimT(x,sa, add_axis = 1, equal_shape = False): 
    """
    ...
    if x is None:
        ...
        if (sx == sa):
            ...
            if ((lsx == 1) | (sx == (1,sa[v3_axis])) | (sx == (sa[v3_axis],1))): 
                if (sx == (1,sa[v3_axis])):
                    x = x.T
                if lsx != lsa:
                    x = np.expand_dims(x,x.ndim-1)
            elif (lsx == 2):
                if (lsa == 3):
...
def xyz_to_labT(data,xyzw = None, cieobs = _cieobs):
    """ 
    ...
    xyzw = todimT(xyzw, data.shape)

    # get and normalize (X,Y,Z) to white point:
    Xr,Yr,Zr = asplitT(data/xyzw)
...
def lab_to_xyzT(data,xyzw = None, cieobs = _cieobs):
    """ 
...
def xyz_to_Ydlep(data, cieobs = _cieobs, xyzw = np.array([[100.0,100.0,100.0]])):
    ...
    # convert xyz to Yxy:
    Yxy = xyz_to_YxyT(data)
    Yxyw = xyz_to_YxyT(xyzw)

    # get spectrum locus Y,x,y and wavelengths:
    SL = _cmf['bar'][cieobs]

    wlsl = SL[0]
    Yxysl = xyz_to_YxyT(SL[1:4].T)
    ...
    dominantwavelength = np.zeros((list(data.shape[0:2]) + [1]))
...
def Ydlep_to_xyz(data, cieobs = _cieobs, xyzw = np.array([[100.0,100.0,100.0]])):
    ...
    #split:
    Y, dom, pur = asplit(data)
    Yw,xw,yw = asplit(Yxyw)
    Ywo,xwo,ywo = asplit(Yxywo)
    Ysl,xsl,ysl = asplit(Yxysl)
    
    # loop over longest dim:
    x = np.zeros((list(Y.shape[0:2])+[1]))
    y = x.copy()
    for i in range(data.shape[0]):
        ...
        # calculate intersection of line through white point and test point and purple line:
        xy = np.hstack((x_dom_wl,y_dom_wl))
        xyw = np.hstack((xw,yw))
        xypl1 = np.hstack((xsl[0,None],ysl[0,None]))
        xypl2 = np.hstack((xsl[-1,None],ysl[-1,None])) 
        da = (xy-xyw)
    ...
    xyz =  Yxy_to_xyz(np.dstack((data[:,:,0],x + xwo, y + ywo)))  
    if axes12flipped == True:
