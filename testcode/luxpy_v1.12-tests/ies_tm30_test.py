...
from luxpy import getwlr, xyz_to_cct, _RFL, spd_to_xyz, xyz_to_Yuv60, _S012_DAYLIGHTPHASE, cat, math, cri
...
def _daylightphase(ccts, wl, add_wl = True, round_Mi_to_cie_recommended = False):
    ...
    if round_Mi_to_cie_recommended: 
        rr = 3
        M1, M2 = np.round(M1,rr), np.round(M2,rr) # round M1, M2 to 3 decimals (CIE recommendation):
...
def cri_ref(ccts, wl, cieobs = "1931_2", n = 1, c2 = None, round_daylightphase_Mi_to_cie_recommended = False, add_wl = True):
...
def _sign_of_Duv(uv, u1,v1,d1,u3,v3,d3):
    ...
    d_tri = np.atleast_1d(d_tri)
    d_tri[d_tri == 0] = 1e-16 # avoid div-by-zero
...
# --- IES TM30-18/24 ---------------------------------------------------------
def _spd_to_jabt_jabr(St, use_1nm_rfls = True, round_daylightphase_Mi_to_cie_recommended = False, verbosity = 0):
...
def spd_to_iesrf(St, use_1nm_rfls = True, round_daylightphase_Mi_to_cie_recommended = False, verbosity = 0):
...
def spd_to_iestm30(spd, use_1nm_rfls = True, round_daylightphase_Mi_to_cie_recommended = False, verbosity = 0):
...
import luxpy as lx
def _spd_to_jabt_jabr_lx(St, use_1nm_rfls = True, round_daylightphase_Mi_to_cie_recommended = False, verbosity = 0):
...
def spd_to_iesrf_lx(St, use_1nm_rfls = True, round_daylightphase_Mi_to_cie_recommended = False, verbosity = 0):
...
def spd_to_iestm30_lx(spd, use_1nm_rfls = True, round_daylightphase_Mi_to_cie_recommended = False, verbosity = 0):
...
if __name__ == '__main__':
    ...
    #tmp2 = pd.read_excel(file, sheet_name = 'Y2norm', skiprows = 19)
    #tmp2 = pd.read_excel(file, sheet_name = 'roundM1M2', skiprows = 19)
    ...
    wl = [360,830,1]
    cctd65,duvd65 = xyz_to_cct(xyzd65, wl = wl, out = 'cct,duv', force_tolerance = True)
    
    Sd = _daylightphase(cctd65[0], wl, round_Mi_to_cie_recommended=False)
    Sd_round = _daylightphase(cctd65[0], wl, round_Mi_to_cie_recommended=True)
    
    xyzd65_ = spd_to_xyz(Sd)
    xyzd65_round = spd_to_xyz(Sd_round)
    cctd65_,duvd65_ = xyz_to_cct(xyzd65_, wl = wl, out = 'cct,duv', force_tolerance = True)
    print(cctd65,cctd65_,cctd65-cctd65_)
    cctd65_round,duvd65_round = xyz_to_cct(xyzd65_round, wl = wl, out = 'cct,duv', force_tolerance = True)
    ...
    # spdi = lx.math.interp1_sprague_cie224_2017(spd[0],spd[1:], wln)
    # spdi2 = lx.cie_interp(spd,wln,kind = 'sprague_cie224_2017', sprague_allowed=True)
    # plt.plot(spd[0],spd[1],'b-')
    # plt.plot(wln,spdi[0],'g--')
    # plt.plot(wln,spdi2[1],'r:')