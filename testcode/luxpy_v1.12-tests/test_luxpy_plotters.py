...
#------------------------------------------------------------------------
# Set NumPy to ignore divide-by-zero globally
#np.seterr(divide='ignore', invalid='ignore') # for some spectra, we have lms = [0,0,0], which leads to a division by zero when calculating the Dt transfer function for chromatic adaptation
...
from luxpy import _CIE_ILLUMINANTS, _CIEOBS
...
if __name__ == '__main__':
    ...
    lx.plotSL( Y_SL = 100,
            ...
            cieobs="1931_2",
            D65=True,
            diagram_lightness=0.5,
            #cspace_pars={"xyzw": lx.spd_to_xyz(lx._CIE_ILLUMINANTS["D65"])},
        )
    plt.ylim([-1000,200])
    

    # Calculate SL in CIELAB:
    SL = _CMF['1931_2']['bar'].copy()
    wl, SL = SL[0], SL[1:4].T
    xyzSL = 100.0*SL/(SL[:,1,None] + _EPS) 
    xyzSL = xyzSL[xyzSL.sum(axis=1)>0,:] # avoid div by zero in xyz-to-Yxy conversion
    labSL = xyz_to_lab(xyzSL)

    plambdamax = SL[:,0].argmax()
    labSL = np.vstack((labSL[:(plambdamax+1),:],labSL[0])) # add lowest wavelength data and go to max of gamut in x (there is a reversal for some cmf set wavelengths >~700 nm!)
    Y,x,y = asplit(labSL)
    abSL = np.vstack((x,y)).T

    #plt.figure()
    #plt.plot(labSL[...,1],labSL[...,2],'r-')
